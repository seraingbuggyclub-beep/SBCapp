...
/**
 * Commande Frigo Libre-service (Passée par le membre en direct)
 */
export async function createSelfServiceOrder(input: {
  ...
  try {
    ...
    const actualBuyerId = input.buyerId || user.id;
    ...
    if (input.paymentMethod === 'WALLET') {
      ...
      if (currentWallet < totalAmount) {
        return {
          success: false,
          error: `Solde portefeuille insuffisant (${currentWallet.toFixed(2)} € disponible). Vous pouvez choisir l'option 'Mettre sur mon Ardoise' ou Payconiq.`,
        };
      }
    ...
    for (const l of orderLines) {
      ...
      await Promise.all([
        ...
        supabase.from('bar_stock_movements').insert({
          ...
          quantity: -l.quantity,
          cost_price_at_time: l.cost_price,
          reason: `Libre-service frigo membre #${actualBuyerId.slice(0, 6)}`,
        }),
      ]);
    }
...
export async function getMembersBalancesList(): Promise<{
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .or('wallet_balance.gt.0,tab_balance.gt.0')
